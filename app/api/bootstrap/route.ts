...
import { completeTopicHashtags, contentInclude, db, ensureSeed, getContentImages, getPersonas, parseHashtags, parseImageReferences, parseImageReferenceSources } from "@/lib/db";
...
export async function GET() {
  ...
  return NextResponse.json({ settings, categories, templates, personas, contents: contents.map((content) => {
    ...
    return { ...content, hashtags: completeTopicHashtags(parseHashtags(content.hashtags), content.topic, content.title), imageReferences, imageReferenceSources: parseImageReferenceSources(content.imageReferenceSources, imageReferences), images: images.filter((image) => image.contentId === content.id) };
  }) });
}
