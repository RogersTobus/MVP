...
import { completeTopicHashtags, contentInclude, db, parseHashtags, parseImageReferences, parseImageReferenceSources } from "@/lib/db";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ ...updated, hashtags: completeTopicHashtags(parseHashtags(updated.hashtags), updated.topic, updated.title), imageReferences, imageReferenceSources: parseImageReferenceSources(updated.imageReferenceSources, imageReferences) });
  } catch (error) {
