...
import { completeTopicHashtags, contentInclude, db, parseHashtags } from "@/lib/db";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await new NaverPublisher(settings.naverBlogId, settings.chromeDebugUrl).prepareDraft({
      ...
      hashtags: completeTopicHashtags(parseHashtags(content.hashtags), content.topic, content.title),
