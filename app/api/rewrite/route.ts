...
import { completeTopicHashtags, contentInclude, db, getContentImages, parseImageReferences, parseImageReferenceSources } from "@/lib/db";
...
export async function POST(request: Request) {
  try {
    ...
    await db.$transaction([
      ...
      db.content.update({ where: { id: content.id }, data: { title: generated.title, summary: generated.summary, hashtags: JSON.stringify(completeTopicHashtags(generated.hashtags, content.topic, generated.title)), body: formatted.filter((_, index) => input.blocks[index]?.type !== "image").map((block) => block.text).join("\n\n"), status: "draft" } }),
    ]);
    ...
    return NextResponse.json({ ...updated, hashtags: completeTopicHashtags(generated.hashtags, content.topic, generated.title), images: await getContentImages(updated.id), imageReferences, imageReferenceSources: parseImageReferenceSources(updated.imageReferenceSources, imageReferences) });
  } catch (error) {
